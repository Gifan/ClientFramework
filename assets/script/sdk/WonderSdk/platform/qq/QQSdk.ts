...
declare let qq: any;
export class QQSdk extends BaseSdk {
    private _bannerAd: any = null;
    public init(appId: string) {
        super.init(appId);
        qq.updateShareMenu && qq.updateShareMenu({ withShareTicket: false });
    ...
    public showVideoAD(adId: string, onPlayEnd?: (code: VideoAdCode, msg?: string) => void): void {
        ...
        if (!videoAd) { onPlayEnd && onPlayEnd(VideoAdCode.NOT_SUPPORT, "不支持视频广告, 请更新QQ版本"); return; }
    ...
    public createAppBox(adId: string, node?: any) {
        ...
        this._appBox.load().then(() => {
            console.log("qq appbox load success");
