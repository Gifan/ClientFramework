...
export default class SinaChannelCtrler implements IChannelCtrler {

    data: ChannelData;
    constructor(defaultChannel: string) {
        let lauchOpts = wb.getGameLaunchOptionSync();
        console.log("[微信渠道][微信启动参数]", lauchOpts);
        this.data = this._tryGetWbChannel(lauchOpts.query)
            || this._tryGetWbChannel(lauchOpts.referrerInfo && lauchOpts.referrerInfo.extraData)
    ...
    protected _tryGetWbChannel(obj: LaunchQuery) {
        ...
        } catch (e) { console.error("[WbChannelHandler][tryGetWbChannel]", e); }
        return data;
    }
