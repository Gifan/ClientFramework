import { CallbackObject, NetData, ISocket, INetworkTips, IProtocolHelper, RequestObject, NetCallFunc } from "./NetInterface";
import { Log } from "../Log";
...
export class NetNode {
    ...
    protected _connectedCallback: CheckFunc = null;
    protected _disconnectCallback: BoolFunc = null;
    protected _callbackExecuter: ExecuterFunc = null;
    ...
    protected _receiveTime: number = 1000000;                               // 多久没收到数据断开
    ...
    protected onConnected(event) {
        ...
        Log.log("NetNode onConnected! state =" + this._state);
    }

    // 连接验证成功，进入工作状态
    protected onChecked() {
        Log.log("NetNode onChecked!")
        ...
        // 重发待发送信息
        Log.log(`NetNode flush ${this._requests.length} request`)
    ...
    protected onMessage(msg): void {
        // Log.log(`NetNode onMessage status = ` + this._state);
        // 进行头部的校验（实际包长与头部长度是否匹配）
        let result = new Uint8Array(msg);
        ...
        // 触发消息执行
        let rspCmd = this._protocolHelper.getPackageId(result);
        Log.log(`NetNode onMessage rspCmd = ` + rspCmd);
        // 优先触发request队列
        if (this._requests.length > 0) {
            for (let reqIdx in this._requests) {
                let req = this._requests[reqIdx];
                if (req.rspCmd == rspCmd) {
                    Log.log(`NetNode execute request rspcmd ${rspCmd}`);
                    this._callbackExecuter(req.rspObject, result);
                    this._requests.splice(parseInt(reqIdx), 1);
                    break;
                }
            }
            Log.log(`NetNode still has ${this._requests.length} request watting`);
            if (this._requests.length == 0) {
        ...
        if (null != listeners) {
            for (const rsp of listeners) {
                Log.log(`NetNode execute listener cmd ${rspCmd}`);
                this._callbackExecuter(rsp, result);
            }
        }
    }
    ...
    public send(buf: NetData, force: boolean = false): boolean {
        if (this._state == NetNodeState.Working || force) {
            ...
            Log.log("NetNode socket is busy, push to send buffer, current state is " + this._state);
            return true;
        } else {
    ...
    public request(buf: NetData, rspCmd: number, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        ...
        Log.log(`NetNode request with timeout for ${rspCmd}`);
        // 进入发送缓存列表
        this._requests.push({
    ...
    public requestUnique(buf: NetData, rspCmd: number, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false): boolean {
        for (let i = 0; i < this._requests.length; ++i) {
            if (this._requests[i].rspCmd == rspCmd) {
                Log.log(`NetNode requestUnique faile for ${rspCmd}`);
                return false;
            }
        }
    ...
    public setResponseHandler(cmd: number, callback: NetCallFunc, target?: any): boolean {
        if (callback == null) {
            Log.error(`NetNode setResponseHandler error ${cmd}`);
            return false;
        }
    ...
    protected resetHearbeatTimer() {
        ...
        this._keepAliveTimer = setTimeout(() => {
            Log.log("NetNode keepAliveTimer send Hearbeat")
